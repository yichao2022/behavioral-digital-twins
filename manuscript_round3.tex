...
Table~\ref{tab:behavioral_alignment} summarizes behavioral consistency. EFR reduced MSE by 94\% (Qwen2.5-72B: 0.0482 $\to$ 0.0030), MAE by 75\% (0.188 $\to$ 0.047), and waiting-time MVR from 62.5\% to 4.2\% on the 64-state grid. On a 200-scenario held-out set, EFR log loss (0.703) was lower than unconstrained LLM (0.815) and Brier score (0.255 vs.\ 0.308), with the largest gains at high-confidence LLM signals.
...
\paragraph*{Pre-empted limitations.} We highlight four design choices that, in our judgment, deserve explicit acknowledgement before external review. \emph{(i) Wuhan DCE, 2023 collection, single-jurisdiction sample.} The empirical frontier is estimated from one DCE fielded in a single Chinese city during the post-acute phase of the COVID-19 pandemic; preference stability across pandemic phases is not established. Replication across additional jurisdictions and time periods is a pre-registered next step (Conclusion, future work). \emph{(ii) Mixed-logit seed and parameter robustness.} The static frontier depends on a single mixed-logit fit; we report point estimates without bootstrap-resampled frontier re-estimation. A reviewer reproducible run with a re-estimated frontier is provided at the public repository. \emph{(iii) \(\lambda = 0.25\) post hoc.} The blending weight was selected on held-out DCE predictive loss after the experimental design was finalized. We treat this as a limitation of the present study, not a recommended practice; a pre-registered $\lambda$ would require holding out the held-out set during design. \emph{(iv) Inference-time regularization vs.\ retraining.} EFR is a convex combination of two streams, not a learned gate; non-convex anchors (mixture-of-experts gating, Bayesian shrinkage toward $P_{\mathrm{emp}}$ priors) remain future work.
...
EFR trades a small within-design accuracy loss (pure-DCE log loss 0.620; EFR 0.703) for narrative-policy evaluation. The NDS pilot ($n=2$) corroborates: isotonic NDS $\rho = -0.943$ vs.\ BDT $\rho = +0.894$ for DeepSeek V4. \textbf{Figure~\ref{fig:ood_performance}} consolidates the four OOD metrics (MVR-Wait, Spearman $\rho$, subgroup heterogeneity, PRC) for Qwen-max and DeepSeek V4, showing that BDT dominates both the unconstrained LLM and the non-anchored NDS baseline on every reported dimension.
...
Table~\ref{tab:bootstrap_prc} reports the exact binomial intervals used for Policy Ranking Consistency (PRC). These intervals are computed over the 15 unordered policy-pair comparisons. The Qwen2.5-72B Static-BDT interval provides the strongest evidence of improved ranking consistency. DeepSeek V4 improves in point estimate, but its interval is wide. MiroThinker-1.7-mini also improves in point estimate, with an interval that remains relatively wide because the total number of policy-pair comparisons is small.
