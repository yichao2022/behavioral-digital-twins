...
\textbf{Results:} On the only non-circular benchmark (held-out DCE predictions), EFR log loss (0.703) is comparable to pure-DCE (0.688; $\Delta = 0.015$). The unconstrained LLM violated empirical waiting-time monotonicity in 62.5\% of in-design scenarios (30.2\% after EFR for Qwen2.5-72B; 7.3\% and 8.3\% for DeepSeek V4 and MiroThinker-1.7-mini, respectively). Frontier-alignment $\rho(\pi_{\text{EFR}}, P_{\text{emp}})$ is reported as a diagnostic: observed rank alignment exceeds what a construction-only null (uniform $U[0,1]$ replacing the LLM component; mean $\rho = 0.367$, 95\% CI $[0.147, 0.566]$) would produce, with Qwen2.5-72B attaining $\rho = 0.691$, a $z$-score of $3.04$ above the null mean. Because the frontier-deviation MSE satisfies $\mathrm{MSE}(\pi_{\text{EFR}}, P_{\text{emp}}) = \lambda^2\,\mathrm{MSE}(\bar\pi_{\text{LLM}}, P_{\text{emp}}) \le \lambda^2$, the reduction in MSE is algebraically induced by the convex blend; the pure-noise experiment is therefore most informative for rank alignment rather than MSE.
...
Section~\ref{sec:stage1} describes the DCE design and empirical frontier; Section~\ref{sec:stage3} presents the EFR framework. Section~\ref{sec:setup} reports results across the 64-state grid and out-of-design evaluation; Section~\ref{sec:conclusion} concludes.
...
We evaluated EFR using three open-source LLMs (Qwen2.5-72B-Instruct, DeepSeek V4, MiroThinker 70B) via Hugging Face inference API with deterministic decoding (temperature=0), identical system prompts. The empirical choice frontier was estimated from a corrected 6-parameter grouped/conditional logit with five attribute coefficients and an opt-out alternative-specific constant on the full Wuhan DCE sample ($N=1{,}027$ respondents) on vaccination timing (ISPOR good-research-practices~\citep{Johnson2013DCE,Hauber2016DCE}).
...
\begin{table*}[t]
\centering
\caption{Behavioral consistency with the empirical reference before and after EFR. The empirical frontier is the corrected 6-parameter grouped/conditional logit (five attribute coefficients plus an opt-out alternative-specific constant) on the full DCE sample; $P_{\mathrm{emp}}$ range $[0.47, 0.64]$ across the 64-state grid; respondent-cluster bootstrap 95\% CIs in Appendix~\ref{app:bootstrap}). \emph{Spearman's $\rho$ is reported as a diagnostic only} because the EFR construction with $\lambda = 0.25$ mechanically bounds it; a pure-noise LLM null attains $\rho \approx 0.37$ on this frontier (Appendix~\ref{sec:null_distribution}). Compared with the 5-parameter frontier (range $[0.27, 0.71]$), the narrower $P_{\mathrm{emp}}$ range under the corrected specification weakens the in-design contrast that EFR can supply: the post-EFR MVR-Wait is higher (e.g., 30.2\% for Qwen2.5-72B vs.\ 4.2\% under the 5-parameter frontier) while MSE/MAE reductions remain large. Held-out DCE log loss is unchanged (0.703) because log-loss on the held-out set is insensitive to the in-design $P_{\mathrm{emp}}$ range.}
...
Table~\ref{tab:behavioral_alignment} summarizes behavioral consistency. EFR reduced MSE by 94\% (Qwen2.5-72B: 0.0258 $\to$ 0.0016), MAE by 75\% (0.127 $\to$ 0.032), and waiting-time MVR from 62.5\% to 30.2\% on the 64-state grid for Qwen2.5-72B; DeepSeek V4 and MiroThinker-1.7-mini achieved similar MSE/MAD reductions with MVR falling to 7.3\% and 8.3\%, respectively. On a 200-scenario held-out set, EFR log loss (0.703) was lower than unconstrained LLM (0.815) and Brier score (0.255 vs.\ 0.308), with the largest gains at high-confidence LLM signals.
...
The differences became more pronounced in the extended scenarios (Panel B). Rich textual narratives containing fictional incentives and complex policy descriptions reduced the rank agreement of the unconstrained LLMs to approximately zero for both models. Among the two wait-varying pairs in Panel B, BDT maintained zero monotonicity violations (0/2); the twelve narrative-only pairs are reported as MAD and $\rho$ only. Rank agreement with the empirical reference was $\rho = 0.93$ for Qwen-max and $\rho = 0.88$ for DeepSeek V4. These results indicate that empirical-frontier regularization continues to preserve waiting-time monotonicity and rank agreement with the empirical reference even when language models are required to interpret policy narratives beyond the original DCE design.
...
EFR improves behavioral consistency of LLM-based synthetic agents without retraining. Three findings emerge. \emph{First}, unconstrained LLMs produce biased choice probabilities: waiting-time monotonicity violations ranged 10.4--62.5\% on the 64-state grid; rank correlation with the empirical frontier was weakly positive (Qwen2.5-72B: $\rho = 0.061$; DeepSeek V4: 0.304; MiroThinker-1.7-mini: 0.531). \emph{Second}, EFR reduced these violations to 7.3--30.2\% across the three LLMs. On the only non-circular benchmark (held-out DCE), EFR log loss (0.703) is comparable to pure-DCE (0.688; $\Delta = 0.015$). \emph{Third}, the policy-ranking improvement transfers: PRC rose 0.333 $\to$ 0.867 for Qwen2.5-72B and 0.50 $\to$ 0.80 for MiroThinker-1.7-mini.
...
\paragraph*{Pre-empted limitations.} We highlight four design choices that, in our judgment, deserve explicit acknowledgement before external review. \emph{(i) Wuhan DCE, single-jurisdiction sample.} The empirical frontier is estimated from one DCE fielded in a single Chinese city (collection period reported in the OSF repository); preference stability across pandemic phases is not established. Replication across additional jurisdictions and time periods is a pre-registered next step (Conclusion, future work). \emph{(ii) Conditional-logit specification and frontier stability.} The static frontier uses a 6-parameter grouped/conditional logit (five attribute coefficients plus an opt-out alternative-specific constant) on the full DCE sample, not a random-coefficients mixed logit. Respondent-cluster bootstrap ($2{,}000$ replicates) confirms frontier stability: Spearman $\rho$ between bootstrap and canonical frontier has mean $0.988$ (median $0.993$; 95\% CI $[0.948, 1.000]$) and all six coefficients retain their sign in $\geq 97.7\%$ of replicates (Appendix~\ref{app:bootstrap}). The corrected opt-out ASC narrows $P_{\mathrm{emp}}$ to $[0.47, 0.64]$; we trade in-design contrast for a behaviorally honest specification. \emph{(iii) \(\lambda = 0.25\) post hoc.} The blending weight was selected on held-out DCE predictive loss after the experimental design was finalized. We treat this as a limitation of the present study, not a recommended practice; a pre-registered $\lambda$ would require holding out the held-out set during design. \emph{(iv) Inference-time regularization vs.\ retraining.} EFR is a convex combination of two streams, not a learned gate; non-convex anchors (mixture-of-experts gating, Bayesian shrinkage toward $P_{\mathrm{emp}}$ priors) remain future work.
...
\item \textbf{Table S2.} Conditional-logit estimates (corrected 6-parameter specification) used to construct the empirical choice frontier.
...
\textbf{B. Figures}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{Figure 2.} Data-scaling ablation: Static-BDT Anchor ($\lambda = 0.25$) evaluated across DCE training shares (30\%, 50\%, 70\%, 100\%); companion to the data-scaling narrative in Section~\ref{sec:data_scaling}.
\end{itemize}
...
To interpret the frontier-alignment Spearman $\rho(\pi_{\text{EFR}}, P_{\text{emp}})$ reported in Table~\ref{tab:behavioral_alignment} and Table~\ref{tab:out_of_design}, we generated a Monte Carlo null distribution by replacing the LLM component with a uniform random draw $U[0, 1]$ on each of 10{,}000 replicates, then computing $\rho(0.25 \cdot U + 0.75 \cdot P_{\text{emp}},\ P_{\text{emp}})$ against the empirical $P_{\text{emp}}$ surface. The 64-state frontier null (corrected 6-parameter specification, $P_{\text{emp}}$ range $[0.47, 0.64]$) has mean $\rho = 0.367$ (SD $0.107$; 95\% CI $[0.147, 0.566]$); EFR with Qwen2.5-72B attains $\rho = 0.691$, a $z$-score of $3.04$ above the null mean. The 8-pair OOD-narrative null ($\rho \approx 0.89$) reflects the narrower narrative-pair surface and is reported as a separate diagnostic in Table~\ref{tab:out_of_design}. The reported frontier-alignment values (Qwen2.5-72B: $\rho = 0.691$; OOD panels: $\rho = 0.68$--$0.87$) lie above the null mean; the OOD values are mechanically dominated by the $(1-\lambda)$ weight on $P_{\text{emp}}$ and should be read as a structural property of the convex anchor rather than a behavioral finding. The fairer benchmark---held-out DCE log loss---is reported in Section~\ref{sec:heldout_validation}.
...
\]
where the ASC is fixed to 1 for the opt-out alternative and 0 for non-opt-out alternatives (i.e., a constant enters only the opt-out alternative's utility). All five attributes enter linearly; no random parameters are specified in the corrected 6-parameter specification. Under conditional logit, the choice probability of accepting policy $x$ against the opt-out alternative is:
\[
...
\]
where $\bar\beta$ is the MLE coefficient vector. The 64-state grid is constructed by varying waiting time, vaccine efficacy, and side-effect risk across four levels each while fixing cash incentives and vaccine origin at their reference levels (0 RMB and reference framing). Including the opt-out ASC narrows the resulting $P_{\mathrm{emp}}$ range to $[0.47, 0.64]$ across the 64-state grid (versus $[0.27, 0.71]$ under the 5-parameter specification without the ASC).
...
\midrule
Unconstrained LLM & 0.0258 & 0.6250 & 0.5521 & $0.06$ \\
Rule-Prompted LLM & 0.0220 & 0.0625 & 0.3021 & 0.68 \\
Static-BDT Anchor $(\lambda=0.25)$ & 0.0016 & 0.3021 & 0.4688 & 0.69 \\
\bottomrule
\end{tabular}
\vspace{0.4em}
\begin{minipage}{0.95\textwidth}
...
\begin{table}[htbp]
\centering
\caption{$\lambda$-sensitivity for frontier-alignment diagnostics on the 64-state grid (corrected 6-parameter specification; $P_{\text{emp}}$ range $[0.47, 0.64]$). Metrics are evaluated against the full-sample empirical frontier $P_{\text{static}}$.}
...
0.10 & 0.0003 & 0.0127 & 0.0729 & 0.2604 & 0.9372 &  \\
0.20 & 0.0010 & 0.0254 & 0.2396 & 0.4375 & 0.7730 &  \\
0.25\(^*\) & 0.0016 & 0.0318 & 0.3021 & 0.4688 & 0.6908 & Main \\
0.30 & 0.0023 & 0.0381 & 0.3854 & 0.4896 & 0.5971 &  \\
0.40 & 0.0041 & 0.0508 & 0.4688 & 0.5208 & 0.4333 &  \\
0.50 & 0.0065 & 0.0635 & 0.5104 & 0.5312 & 0.3255 &  \\
0.60 & 0.0093 & 0.0762 & 0.5208 & 0.5312 & 0.2472 &  \\
0.70 & 0.0127 & 0.0890 & 0.5521 & 0.5417 & 0.1752 &  \\
0.75 & 0.0145 & 0.0953 & 0.5729 & 0.5417 & 0.1502 &  \\
0.80 & 0.0165 & 0.1017 & 0.6042 & 0.5417 & 0.1208 &  \\
0.90 & 0.0209 & 0.1144 & 0.6042 & 0.5521 & 0.0934 &  \\
1.00 & 0.0258 & 0.1271 & 0.6250 & 0.5521 & 0.0608 & Unconstrained LLM \\
\bottomrule
\end{tabular}
...
\begin{table}[htbp]
\centering
\caption{Data-scaling ablation: Static-BDT Anchor (\(\lambda = 0.25\)) evaluated against the full-sample 6-parameter frontier (reported at seed 1 of 5). The own-frontier benchmark (each subsample's re-estimated frontier) is qualitatively similar; under the corrected specification, MSE remains near $0.011$ across the $30\%$--$70\%$ subsamples and $\rho$ remains above $0.6$.}
\label{tab:data_scaling_benchmarks}
\begin{tabular}{llccc}
\toprule
DCE share & Benchmark & MSE & MVR-Wait & Spearman \(\rho\) \\
\midrule
30\% & Full sample & 0.0097 & 0.3542 & 0.5941 \\
50\% & Full sample & 0.0072 & 0.3646 & 0.6124 \\
70\% & Full sample & 0.0085 & 0.3438 & 0.6582 \\
100\% & Full sample & 0.0016 & 0.3021 & 0.6908 \\
\bottomrule
\end{tabular}
\end{table}

Two patterns emerge. Against the full-sample frontier, all three metrics improve as more DCE respondents are used to estimate the empirical frontier. The sharp improvement at $100\%$ reflects the transition from a subsample-estimated conditional-logit frontier to the full-sample frontier, indicating that BDT's precision is partly gated by the quality and specification of the empirical frontier. The own-frontier benchmark (omitted from the table for brevity; results available in the replication archive) shows that the anchoring mechanism remains internally consistent: MSE stays near $0.011$ across the $30\%$--$70\%$ subsamples, MVR-Wait stays near $0.35$, and Spearman $\rho$ stays above $0.6$, indicating that BDT stays close to whichever frontier it is given even when that frontier is estimated from limited data.
...
\midrule
DeepSeek V4 & Unconstrained LLM & 2/15 & 0.133 & [0.017, 0.405] \\
DeepSeek V4 & EFR & 9/15 & 0.600 & [0.323, 0.837] \\
\midrule
...
\begin{minipage}{0.95\textwidth}
\small
\textit{Note.} PRC measures the proportion of intervention pairs ranked in the same order as the Pure-DCE reference. The policy set contains six interventions, yielding 15 unordered policy pairs. Brackets report exact binomial 95\% confidence intervals. One-sided exact binomial tests were performed against the random-ranking baseline (PRC = 0.5). Benjamini--Hochberg correction was applied across the three EFR model comparisons (Qwen2.5-72B, DeepSeek V4, and MiroThinker-1.7-mini); Qwen2.5-72B and MiroThinker-1.7-mini remained significant after correction.
\end{minipage}
\end{table}
...
\subsection{Corrected 6-Parameter Frontier Stability}
\label{app:bootstrap_6param}

The corrected 6-parameter frontier (five attributes plus an opt-out alternative-specific constant) was re-estimated under a 2{,}000-replicate respondent-level cluster bootstrap on the full DCE sample. The bootstrap frontier was compared against the canonical full-sample frontier on the 64-state grid using Spearman $\rho$. Mean $\rho = 0.988$ (median $0.993$; 95\% CI $[0.948, 1.000]$) and mean MSE between bootstrap and canonical frontier probability vectors $= 1.41 \times 10^{-4}$ (median $9.04 \times 10^{-5}$). All six coefficients retained their canonical sign in $\geq 97.7\%$ of replicates; the side-effect coefficient is the least stable attribute effect, retaining its negative sign in $97.65\%$ of bootstrap replicates, consistent with its comparatively weak empirical signal (canonical $\hat{\beta}_{\text{SE}} = -0.036$; 95\% CI $[-0.074, -0.0003]$). Coefficient point estimates, bootstrap means, 95\% CIs, and sign-retention rates for the corrected 6-parameter specification are reported in the main text Section~\ref{sec:setup} and the conditional-logit estimate table; Spearman $\rho$ and MSE summaries above are the primary stability diagnostics used in this appendix.

The estimated 6-parameter empirical frontier was highly stable under respondent-level resampling, with a mean Spearman correlation of $0.988$ relative to the full-sample frontier. The side-effect coefficient was the least stable attribute effect, retaining its negative sign in 97.65\% of bootstrap replicates, consistent with its comparatively weak empirical signal.

\subsection{Held-Out Prediction Uncertainty}

Table~\ref{tab:bootstrap_heldout} reports bootstrap intervals for the held-out prediction metrics, computed via respondent-level cluster bootstrapping to account for the nested panel structure of DCE choices. On the LLM-matched held-out subset, Static-BDT improves substantially over the unconstrained LLM for both log loss and Brier score. The intervals for Static-BDT and the unconstrained LLM do not overlap for either metric. In contrast, the intervals for Static-BDT and Pure-DCE overlap for both log loss and Brier score, supporting the interpretation that Static-BDT brings raw LLM probabilities close to the structured DCE benchmark but should not be interpreted as outperforming Pure-DCE within the DCE attribute space.
...
\midrule
DeepSeek V4 & Unconstrained LLM & 2/15 & 0.133 & [0.017, 0.405] \\
DeepSeek V4 & Static-BDT Anchor & 9/15 & 0.600 & [0.323, 0.837] \\
\midrule
MiroThinker-1.7-mini & Unconstrained LLM & 8/15 & 0.533 & [0.266, 0.787] \\
