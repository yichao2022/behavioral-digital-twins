...
Large language models (LLMs) are increasingly used as simulated decision-makers for healthcare policy evaluation and simulation~\citep{Park2023a, Mu2024, VonHoene2025}. Their appeal is that they can interpret rich natural-language scenarios that conventional structured decision models cannot directly process. However, linguistic fluency does not guarantee behavioral reliability. For example, in a vaccination policy simulation, an unconstrained LLM may assign higher acceptance to a strategy with a longer waiting time than to an otherwise identical strategy with a shorter waiting time. Such reversals contradict empirically observed preference patterns and can produce unreliable policy rankings.

Current approaches have demonstrated that LLMs can generate coherent responses to policy scenarios, but they do not explicitly enforce empirically validated behavioral constraints~\citep{Aher2022TuringExperiments, Horton2023, santurkar2023whose}. As a result, LLM agents may produce decisions that appear linguistically plausible while violating basic preference relationships implied by observed human behavior, limiting their reliability for healthcare policy simulation.
...
We propose a \textbf{hybrid approach} that anchors LLM-generated choice probabilities to empirical behavioral evidence from DCEs. The method estimates an empirical choice frontier from DCE responses and blends the LLM's raw probability with the frontier through a transparent weighted-averaging procedure. The adjustment is auditable because the raw LLM probability, the empirical frontier value, the blending weight, and the final adjusted probability can all be inspected directly. The method requires no retraining or modification of the underlying LLM. This design allows the same LLM to be anchored using different empirical datasets, enabling domain-specific behavioral grounding while preserving the general capabilities of the foundation model.
...
Section~\ref{sec:stage1} describes the DCE design and empirical frontier. Section~\ref{sec:stage2} introduces the EFR framework. Section~\ref{sec:stage3} reports results across 64-state grid and out-of-design evaluation. Section~\ref{sec:limitations} discusses implications for health policy analysis.
...
Two physician interviews and two rounds of twelve cognitive interviews (ISPOR good-research-practices~\citep{Johnson2013DCE,Bridges2013DCE}) refined the choice-task framing. We acknowledge the absence of a formal advisory board as a limitation, particularly for representing payer and patient perspectives.
...
The final stage applies the adjusted probability to policy evaluation. The adjusted response surface can be queried to compare policy scenarios and estimate changes in expected acceptance.
...
We evaluated EFR using three open-source LLMs (Qwen2.5-72B-Instruct, DeepSeek V4, MiroThinker 70B) accessed via the Hugging Face inference API with deterministic decoding (temperature = 0) , identical system prompts. The empirical choice frontier was estimated from a mixed-logit DCE with $N=1{,}027$ Wuhan respondents on vaccination timing (ISPOR good-research-practices~\citep{Johnson2013DCE,Hauber2016DCE}).
...
Table~\ref{tab:behavioral_alignment} summarizes behavioral consistency before and after EFR. EFR reduced MSE by 94\% (Qwen2.5-72B: 0.0482 $\to$ 0.0030), MAE by 75\% (0.188 $\to$ 0.047), and waiting-time MVR from 62.5\% to 4.2\% on the 64-state grid. On a 200-scenario held-out set, EFR log loss (0.703) was lower than unconstrained LLM (0.756) and Brier score (0.255 vs.\ 0.296), with the largest gains at high-confidence LLM signals. The mechanism is grounding the strongest LLM signals in empirical evidence, not attenuating them.

\clearpageOut-of-Design Policy Scenario Evaluation}
...
EFR also improved policy-ranking consistency (Spearman PRC): Qwen2.5-72B-Instruct 0.333 $\to$ 0.867; DeepSeek V4 0.55 $\to$ 0.60; MiroThinker 70B 0.50 $\to$ 0.80, confirming EFR translates behavioral-consistency gains into policy-level ranking reliability.

\section{Discussion}
\subsection{Principal Findings}

EFR improves behavioral consistency of LLM-based synthetic agents without retraining. Across three open-source LLMs, EFR reduced waiting-time monotonicity violations from 39.6\%--62.5\% to 0\%--4.2\% on a 64-state grid; held-out DCE log loss (0.703) is comparable to pure-DCE (0.688).

\subsection{Methodological Contributions}

Our primary contribution is inference-time empirical regularization, aligning LLM decisions with empirically estimated behavioral structure, without modifying model weights. The design reflects \emph{theory-informed machine learning}: established empirical models provide structural priors for generative AI systems.

A revision-stage stress test (8/13/2026) on eight Panel B scenarios across three smaller open-source models (4,800 prompts) corroborated this view: Qwen2.5-7B produced constant outputs (zero variance, default-answer signature), DeepSeek-V4-Flash achieved 0\% parse on the waiver-language N1 scenario. \emph{These failures should be read as a safety property, not as evidence of LLM value-add}: when LLM output is degenerate, EFR degrades gracefully to pure DCE (the 75\% weight on $P_{\text{emp}}$ dominates). The empirical reference, not the LLM, supplies the behavioral signal.

\subsection{Implications for Health Policy Analysis}
\label{sec:impl_policy}

The EFR framework has three HTA implications: (i) a recent DCE enables low-marginal-cost narrative policy simulation; (ii) the framework extends a single DCE beyond its original design; (iii) the framework is best suited to early-stage screening, not high-stakes final decisions.

\subsection{Value of Information in the EFR Framework}
\label{sec:voi}

EFR is a \emph{cost-ratio} framework (Appendix~\ref{app:voi}): at US\$\,10,000 LLM marginal cost vs.\ US\$\,500,000 typical DCE cost, the $50\times$ ratio applies only when a DCE already exists. The oracle-BDT gap (Table~\ref{tab:evpi}) is a per-decision-maker value gap, not a population-scaled EVPI~\citep{Fenwick2008EVPI,Briggs2006VOI}.

\subsection{Why Not Use the Discrete Choice Model Alone?}
\label{sec:why_not_dce}

EFR sacrifices a small amount of within-design accuracy (pure-DCE log loss 0.620; EFR 0.703) to evaluate unstructured policy narratives outside the DCE design. The pilot NDS comparison ($n=2$ $P_{\text{static}}$ pairs) corroborates the trade-off (isotonic $\rho = -0.943$ vs.\ BDT $\rho = +0.894$ for DeepSeek V3).



\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figure_1_ood_performance.png}
\caption{Out-of-design performance of BDT (\(\lambda=0.25\)) versus NDS (isotonic) and unconstrained LLM. \textbf{(a)} Mean violation rate (MVR-Wait) on 6 paired wait-time comparison groups. \textbf{(b)} Spearman \(\rho\) for policy ranking against the empirical frontier (95\% CI bars). \textbf{(c)} BDT \(\rho\) by demographic subgroup (809-respondent matched subset). \textbf{(d)} Policy ranking consistency (PRC) across the 20 OOD scenarios. BDT achieves the highest \(\rho\) and lowest MVR across both LLMs; subgroup \(\rho\) remains near-unity (\(\geq 0.949\)), indicating that the anchoring benefit transfers across demographic strata.}
\label{fig:ood_performance}
\end{figure}

\subsection{Practical Implementation for Health Policy Analysts}
\label{sec:practical_impl}

Implementation requires no model retraining: an analyst estimates a mixed-logit from an existing DCE~\citep{Johnson2013DCE,Hauber2016DCE}, generates LLM predictions, and combines them through $\pi_{\mathrm{ad}} = \lambda \pi_{\mathrm{LLM}} + (1-\lambda) P_{\mathrm{emp}}$ ($\lambda = 0.25$). The dominant cost is the one-time LLM API access (US\$\$10,000 open-source; US\$\$100,000 commercial), negligible relative to a new US\$\$500,000 DCE.

\subsection{Limitations and Future Directions}\label{sec:limitations}

Five limitations: (i) single Wuhan DCE, single vaccination setting; generalizability requires replication. (ii) EFR inherits any $P_{\text{emp}}$ misspecification. (iii) LLM generalizability (MUST-7): V4 Pro and MiroThinker-1.7-mini unavailable on OpenRouter for OOD, so Qwen2.5-72B and V3 used; revision-stage stress test (8/13/2026, 4,800 prompts) confirmed zero-variance and parsing failures. (iv) $\lambda$ sensitivity grid (Appendix~\ref{app:dce_attributes}) with $\lambda=0.25$ selected on held-out predictive performance. (v) LLM software stack fixed (Hugging Face, temperature=0).

\subsection{Policy Recommendations}\label{sec:policy_rec}

\emph{First}, with a DCE, static-BDT provides a low-marginal-cost path from evidence to narrative policy simulation. \emph{Second}, with no relevant DCE, the framework is \emph{not} a substitute for stated-preference data. \emph{Third}, best for early-stage screening, not high-stakes final decisions.

\subsection{Conclusion}\label{sec:conclusion}

This study proposed EFR, anchoring LLM outputs to DCE estimates without retraining. On the only non-circular benchmark (held-out DCE predictions), EFR log loss (0.703) is comparable to pure-DCE (0.688); the 75% weight on $P_{\text{emp}}$ provides graceful degradation when LLM outputs are unreliable. Across 20 OOD policy narratives, EFR reduced the LLM monotonicity violation rate from 39.6%--62.5% to 0%--4.2%. EFR is a low-marginal-cost pathway from stated-preference evidence to narrative policy simulation, but it is not a stand-alone LLM behavioral simulator: the empirical reference, not the LLM, supplies the behavioral signal.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figure_S1_data_scaling.png}
\caption{
